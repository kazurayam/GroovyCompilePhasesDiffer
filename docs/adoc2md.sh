...
function processFile() {
  ...
  # We named `index_.adoc` rather than `index_.adoc` because GitHub puts precedence to `index_.adoc` over `index.md`. We want `index.md` to be presented first, not `*.adoc`. Therefore we named our adoc file with `*_.adoc` postfix.
